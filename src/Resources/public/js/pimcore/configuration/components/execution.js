...
opendxp.registerNS('opendxp.plugin.pimcoreDataImporterBundle.configuration.components.execution');
opendxp.plugin.pimcoreDataImporterBundle.configuration.components.execution = Class.create({
    ...
    buildPanel: function() {
        ...
        if(!this.form) {
                        ...
                        handler: function() {
                            Ext.Ajax.request({
                                ...
                                success: function (response) {
                                    ...
                                    opendxp.helpers.showNotification(t('success'), t('plugin_pimcore_datahub_data_importer_configpanel_execution_cancel_successful'), 'success');
                                    this.updateProgress();
            ...
            this.configItemRootContainer.on(opendxp.plugin.pimcoreDataImporterBundle.configuration.events.loaderTypeChanged, function(newType) {
                this.currentLoaderType = newType;
                this.updateDisabledState();
            }.bind(this));

            this.configItemRootContainer.on(opendxp.plugin.pimcoreDataImporterBundle.configuration.events.configDirtyChanged, function(dirty) {
        ...
        Ext.Ajax.request({
            ...
            success: function (response) {
                ...
                if (data && data.success) {
                    opendxp.helpers.showNotification(t('success'), t('plugin_pimcore_datahub_data_importer_configpanel_execution_start_successful'), 'success');
                } else {
                    opendxp.helpers.showNotification(t("error"), t('plugin_pimcore_datahub_data_importer_configpanel_execution_start_error'), 'error');
                }
