...
opendxp.registerNS('opendxp.plugin.pimcoreDataImporterBundle.configuration.components.importPreview');
opendxp.plugin.pimcoreDataImporterBundle.configuration.components.importPreview = Class.create({
    ...
    checkValidConfiguration: function(suppressInvalidError) {
        ...
        if(!isValid) {
            if(!suppressInvalidError) {
                opendxp.helpers.showNotification(t('error'), t('plugin_pimcore_datahub_data_importer_configpanel_invalid_config_for_preview'), 'error');
            }
            return false;
        }
        return true;
    ...
    uploadDialog: function() {
        ...
        const url = Routing.generate('opendxp_dataimporter_configdataobject_uploadpreviewdata', {'config_name': this.configName});

        opendxp.helpers.uploadDialog(url, 'Filedata', function() {
            this.updatePreview();
    ...
    updatePreview: function(direction, suppressInvalidError) {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_dataimporter_configdataobject_loaddatapreview'),
            method: 'POST',
            params: {
    ...
    copyPreviewFromDataSource: function() {
        ...
        Ext.Ajax.request({
            url: Routing.generate('opendxp_dataimporter_configdataobject_copypreviewdata'),
            method: 'POST',
            params: {
                config_name: this.configName,
                current_config: currentConfig
