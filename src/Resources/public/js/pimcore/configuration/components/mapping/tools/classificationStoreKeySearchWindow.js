...
opendxp.registerNS('opendxp.plugin.pimcoreDataImporterBundle.configuration.components.mapping.tools.classificationStoreKeySearchWindow');

opendxp.plugin.pimcoreDataImporterBundle.configuration.components.mapping.tools.classificationStoreKeySearchWindow = Class.create({
    ...
    buildPanel: function (classId, fieldname, transformationResultType) {
        ...
        const proxy = {
            type: 'ajax',
            url: Routing.generate('opendxp_dataimporter_configdataobject_loaddataobjectclassificationstorekeys'),
            reader: {
                type: 'json',
                rootProperty: 'data',
            },
        ...
        const pageSize = opendxp.helpers.grid.getDefaultPageSize(-1);
        const pagingtoolbar = opendxp.helpers.grid.buildDefaultPagingToolbar(this.store, {pageSize: pageSize});
