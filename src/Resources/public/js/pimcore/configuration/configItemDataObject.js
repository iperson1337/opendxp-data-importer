...
opendxp.registerNS('opendxp.plugin.pimcoreDataImporterBundle.configuration.configItemDataObject');
opendxp.plugin.pimcoreDataImporterBundle.configuration.configItemDataObject = Class.create(opendxp.plugin.datahub.configuration.graphql.configItem, {

    urlSave: Routing.generate('opendxp_dataimporter_configdataobject_save'),
    ...
    save: function () {
        ...
        if (!this.isValid(true)) {
            opendxp.helpers.showNotification(t('error'), t('plugin_pimcore_datahub_data_importer_configpanel_invalid_config'), 'error');
            return;
        }
            ...
            success: function (response) {
                ...
                if (rdata && rdata.success) {
                    ...
                    this.saveOnComplete();
                        ...
                        opendxp.helpers.showNotification(t("error"), t("plugin_pimcore_datahub_configpanel_item_saveerror_permissions"), "error");
                        this.tab.setActiveTab(this.tab.items.length-1);
                } else {
                    opendxp.helpers.showNotification(t("error"), t("plugin_pimcore_datahub_configpanel_item_saveerror"), "error", t(rdata.message));
                }
            }.bind(this)
        });
    },
                    ...
                    items: [
                        {
                            ...
                            name: "type",
                            optionsNamespace: opendxp.plugin.pimcoreDataImporterBundle.configuration.components.loader,
                            ...
                            listeners: {
                                change: function(combo, newValue, oldValue) {
                                    this.tab.fireEvent(
                                        opendxp.plugin.pimcoreDataImporterBundle.configuration.events.loaderTypeChanged,
                                        newValue
                                    );
                                }.bind(this)
                            }
                    ...
                    items: [
                        {
                            ...
                            xtype: 'subsettingscombo',
                            name: 'type',
                            optionsNamespace: opendxp.plugin.pimcoreDataImporterBundle.configuration.components.interpreter,
    ...
    buildImportSettingsTab: function() {
        ...
        const transformationResultHandler = new opendxp.plugin.pimcoreDataImporterBundle.configuration.components.mapping.transformationResultHandler(this.configName, this, null);
        const importPreview = new opendxp.plugin.pimcoreDataImporterBundle.configuration.components.importPreview(this.configName, this, transformationResultHandler);
        this.importSettings = new opendxp.plugin.pimcoreDataImporterBundle.configuration.components.importSettings(this.data, this.tab, transformationResultHandler);
    ...
    buildExecutionTab: function() {
        const execution = new opendxp.plugin.pimcoreDataImporterBundle.configuration.components.execution(this.configName, this.data.executionConfig, this.tab, this.data.loaderConfig.type);
        this.executionForm = execution.buildPanel();
        return this.executionForm;
    },

    buildLoggerTab: function() {
        const loggerTab = new opendxp.plugin.pimcoreDataImporterBundle.configuration.components.logTab(this.configName);
        return loggerTab.getTabPanel();
    },

    updateColumnHeaders: function() {
        Ext.Ajax.request({
            url: Routing.generate('opendxp_dataimporter_configdataobject_loadavailablecolumnheaders'),
            method: 'POST',
            params: {
    ...
    detectedChange: function ($super) {
        ...
        if(this.tab) {
            this.tab.fireEvent(
                opendxp.plugin.pimcoreDataImporterBundle.configuration.events.configDirtyChanged,
                this.dirty
            );
        }
    },
    ...
    saveOnComplete: function () {
        ...
        opendxp.helpers.showNotification(t("success"), t("plugin_pimcore_datahub_configpanel_item_save_success"), "success");
