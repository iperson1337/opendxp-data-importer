...
opendxp.registerNS('opendxp.plugin.opendxpDataImporterBundle.configuration.configItemDataObject');
opendxp.plugin.opendxpDataImporterBundle.configuration.configItemDataObject = Class.create(opendxp.plugin.datahub.configuration.graphql.configItem, {
    ...
    save: function () {
        ...
        if (!this.isValid(true)) {
            opendxp.helpers.showNotification(t('error'), t('plugin_opendxp_datahub_data_importer_configpanel_invalid_config'), 'error');
            return;
        }
    ...
    buildDataSourceTab: function () {
        ...
        this.loaderForm = Ext.create('DataHub.DataImporter.StructuredValueForm', {
            items: [
                {
                    xtype: "fieldset",
                    title: t('plugin_opendxp_datahub_data_importer_configpanel_datasource'),
                    defaults: defaults,
                    items: [
                        {
                            fieldLabel: t("plugin_opendxp_datahub_data_importer_configpanel_datasource_type"),
                            xtype: "subsettingscombo",
                            name: "type",
                            optionsNamespace: opendxp.plugin.opendxpDataImporterBundle.configuration.components.loader,
                            ...
                            listeners: {
                                change: function(combo, newValue, oldValue) {
                                    this.tab.fireEvent(
                                        opendxp.plugin.opendxpDataImporterBundle.configuration.events.loaderTypeChanged,
                                        newValue
                                    );
                                }.bind(this)
                            }
        ...
        this.interpreterForm = Ext.create('DataHub.DataImporter.StructuredValueForm', {
            items: [
                {
                    xtype: 'fieldset',
                    title: t('plugin_opendxp_datahub_data_importer_configpanel_file_format'),
                    defaults: defaults,
                    items: [
                        {
                            fieldLabel: t('plugin_opendxp_datahub_data_importer_configpanel_file_formats_type'),
                            xtype: 'subsettingscombo',
                            name: 'type',
                            optionsNamespace: opendxp.plugin.opendxpDataImporterBundle.configuration.components.interpreter,
    ...
    buildImportSettingsTab: function() {
        ...
        const transformationResultHandler = new opendxp.plugin.opendxpDataImporterBundle.configuration.components.mapping.transformationResultHandler(this.configName, this, null);
        const importPreview = new opendxp.plugin.opendxpDataImporterBundle.configuration.components.importPreview(this.configName, this, transformationResultHandler);
        this.importSettings = new opendxp.plugin.opendxpDataImporterBundle.configuration.components.importSettings(this.data, this.tab, transformationResultHandler);


        return Ext.create('Ext.Panel', {
            title: t('plugin_opendxp_datahub_data_importer_configpanel_import_settings'),
            bodyStyle: 'padding:10px;',
            layout: 'border',
            items: [
    ...
    buildExecutionTab: function() {
        const execution = new opendxp.plugin.opendxpDataImporterBundle.configuration.components.execution(this.configName, this.data.executionConfig, this.tab, this.data.loaderConfig.type);
    ...
    detectedChange: function ($super) {
        ...
        if(this.tab) {
            this.tab.fireEvent(
                opendxp.plugin.opendxpDataImporterBundle.configuration.events.configDirtyChanged,
                this.dirty
            );
        }
    },
