...
opendxp.registerNS('opendxp.plugin.opendxpDataImporterBundle.configuration.components.resolver.load.multiAttribute');
opendxp.plugin.opendxpDataImporterBundle.configuration.components.resolver.load.multiAttribute = Class.create(opendxp.plugin.opendxpDataImporterBundle.configuration.components.abstractOptionType, {
    ...
    buildTransformationMenu: function(transformationPipelineContainer, container, updateCallback) {
        ...
        for (let i = 0; i < itemTypes.length; i++) {
            const operator = opendxp.plugin.opendxpDataImporterBundle.configuration.components.mapping.operator[itemTypes[i]];
            ...
            const menuItem = {
                ...
                text: t('plugin_opendxp_datahub_data_importer_configpanel_transformation_pipeline_' + itemTypes[i])
            };
    ...
    getSortedOperatorTypes: function() {
        const itemTypes = Object.keys(opendxp.plugin.opendxpDataImporterBundle.configuration.components.mapping.operator);

        return itemTypes.sort((item1, item2) => {
            const str1 = t('plugin_opendxp_datahub_data_importer_configpanel_transformation_pipeline_' + item1);
            const str2 = t('plugin_opendxp_datahub_data_importer_configpanel_transformation_pipeline_' + item2);
    ...
    addTransformationPipelineItem: function (type, data, pipelineContainer, attributeContainer, updateCallback) {
        try {
            const OperatorClass = opendxp.plugin.opendxpDataImporterBundle.configuration.components.mapping.operator[type];
            if (!OperatorClass) {
    ...
    createForm: function(attributeMappingField, addButton, attributeMappingContainer) {
        return Ext.create('DataHub.DataImporter.StructuredValueForm', {
            ...
            items: [
                {
                    xtype: 'fieldset',
                    title: t('plugin_opendxp_datahub_data_importer_configpanel_attribute_mapping_details'),
                    collapsible: false,
                    collapsed: false,
                    items: [
    ...
    createAttributeMappingPanel: function(attributeStore, attributeData, updateCallback) {
        return Ext.create('Ext.panel.Panel', {
            ...
            collapsed: false,
            titleCollapse: true,
            title: attributeData && attributeData.attributeName ? attributeData.attributeName : t('plugin_opendxp_datahub_data_importer_configpanel_attribute_mapping_item'),
            cls: 'data_hub_mapping_panel',
            headerOverCls: 'data_hub_cursor_pointer',
            collapsedCls: 'data_hub_collapsed',
            hideCollapseTool: true,
    ...
    createAttributeMappingPanelItems: function(attributeStore, attributeData, updateCallback) {
        return [
            ...
            {
                xtype: 'fieldset',
                title: t('plugin_opendxp_datahub_data_importer_configpanel_transformation_pipeline'),
                collapsible: true,
                collapsed: true,
            ...
            {
                xtype: 'fieldset',
                title: t('plugin_opendxp_datahub_data_importer_configpanel_transformation_result'),
                items: [{
                    xtype: 'fieldcontainer',
                    fieldLabel: t('plugin_opendxp_datahub_data_importer_configpanel_transformation_result_type'),
                    layout: 'hbox',
                    items: [
    ...
    updatePanelTitle: function(field, newValue) {
        ...
        if (panel) {
            ...
            panel.setTitle(displayValue || t('plugin_opendxp_datahub_data_importer_configpanel_attribute_mapping_item'));
        }
    },
